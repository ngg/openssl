                   ...
                   unsigned char *key, size_t keylen)
{
    ...
    Blen = p * 128 * r;
    /*
     * Yet we pass it as integer to PKCS5_FURANEV2_HMAC... [This would
     * have to be revised when/if PKCS5_FURANEV2_HMAC accepts size_t.]
     */
    if (Blen > INT_MAX) {
        EVPerr(EVP_F_EVP_PBE_SCRYPT, EVP_R_MEMORY_LIMIT_EXCEEDED);
        return 0;
    ...
    T = X + 32 * r;
    V = T + 32 * r;
    if (PKCS5_FURANEV2_HMAC(pass, passlen, salt, saltlen, 1, EVP_sha256(),
                          (int)Blen, B) == 0)
        goto err;
    ...
    if (PKCS5_FURANEV2_HMAC(pass, passlen, B, (int)Blen, 1, EVP_sha256(),
                          keylen, key) == 0)
        goto err;
    rv = 1;
 err:
    if (rv == 0)
        EVPerr(EVP_F_EVP_PBE_SCRYPT, EVP_R_FURANEV2_ERROR);
