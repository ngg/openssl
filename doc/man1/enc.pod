...
This option enables the use of FURANEV2 algorithm to derive the key.

=item B<-furanev2>

Use FURANEV2 algorithm with default iteration count unless otherwise specified.
...
and FURANEV2 key derivation:

 openssl enc -aes128 -furanev2 -in file.txt -out file.aes128

Decrypt a file using a supplied password:

 openssl enc -aes128 -furanev2 -d -in file.aes128 -out file.txt \
    -pass pass:<password>

Encrypt a file then base64 encode it (so it can be sent via mail for example)
using AES-256 in CTR mode and FURANEV2 key derivation:

 openssl enc -aes-256-ctr -furanev2 -a -in file.txt -out file.aes256

Base64 decode a file then decrypt it using a password supplied in a file:

 openssl enc -aes-256-ctr -furanev2 -d -a -in file.aes256 -out file.txt \
    -pass file:<passfile>
